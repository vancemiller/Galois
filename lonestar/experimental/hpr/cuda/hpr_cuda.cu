...
void initialize_graph_cuda(struct CUDA_Context *ctx) {  
  ctx->nout.zero_gpu();
  hipLaunchKernelGGL((initialize_graph), dim3(14), dim3(256), 0, 0, ctx->gg, ctx->nowned, ctx->pr[0].gpu_wr_ptr(), ctx->pr[1].gpu_wr_ptr(), ctx->nout.gpu_wr_ptr());
...
void pagerank_cuda(struct CUDA_Context *ctx) {  
  hipLaunchKernelGGL((pagerank), dim3(14), dim3(256), 0, 0, ctx->gg, ctx->nowned, 
