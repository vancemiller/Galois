...
#define VARIABLE_NAME_CHARACTERS                                               \
  "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_"
#define COMPUTE_FILENAME "._compute_kernels"
#define ORCHESTRATOR_FILENAME "._orchestrator_kernel"

// ASSUMPTIONS
#define __GALOIS_PREPROCESS_GLOBAL_VARIABLE_PREFIX__                           \
  "local_" // TODO: detect automatically
...
private:
  ...
  Rewriter&
      rewriter; // FIXME: is this necessary? can ASTContext give source code?
  bool isTopological;
  ...
  std::unordered_set<std::string>
      symbolTable; // FIXME: does not handle scoped variables (nesting with same
                   // variable name)
  std::ofstream Output;
  ...
  unsigned
      conditional; // if (generated) code is enclosed within an if-condition
  bool conditional_pop; // if (generated) code is enclosed within an (generated)
                        // if-condition for pop

public:
  explicit IrGLOperatorVisitor(
      ASTContext* context, Rewriter& R, bool isTopo,
      ...
      : // astContext(context),
        rewriter(R), isTopological(isTopo),
        accumulatorToTypeMap(accumToTypeMap), reducemaxToTypeMap(maxToTypeMap),
        reduceminToTypeMap(minToTypeMap),
        SharedVariablesToTypeMap(sharedVariables),
        KernelToArgumentsMap(kernels), conditional(0), conditional_pop(false) {}
  ...
  std::string FormatCBlock(std::string text) {
    ...
    // replace std::distance(graph->edge_begin(NODE), graph->edge_end(NODE))
    // with getOutDegree(NODE)
    while (1) {
      ...
      if (found != std::string::npos) {
        ...
        std::string replace = "graph.getOutDegree(" + var + ")";
        assert(text.find("graph->edge_end(" + var + ")", end) !=
               std::string::npos);
    ...
    // replace ATOMIC_VAR[NODE].exchange(VALUE) with
    // atomicExch(&ATOMIC_VAR[NODE], VALUE)
    while (1) {
    ...
    findAndReplace(text, " this->",
                   " "); // because IrGL is not aware of the "this" variable
  ...
  virtual bool TraverseDecl(Decl* D) {
    ...
    if (traverse && D && isa<CXXMethodDecl>(D)) {
      ...
      for (auto& it : accumulatorToTypeMap) {
        ...
        bodyString << "CBlock([\"" << name << ".thread_exit<" << br << " >("
                   << name << "_ts)\"], parse = False),\n";
      }
      for (auto& it : reducemaxToTypeMap) {
        auto name = it.first;
        auto type = it.second;
      ...
      KernelToArgumentsMap[funcName] =
          std::make_tuple(globalArguments, arguments, bitsetArguments);
      declString << "],\n[\n";
  ...
  virtual bool TraverseCXXForRangeStmt(CXXForRangeStmt* forStmt) {
    assert(forStmt);
    bool traverse =
        RecursiveASTVisitor<IrGLOperatorVisitor>::TraverseCXXForRangeStmt(
            forStmt);
    if (traverse) {
      bodyString << "]),\n"; // end ForAll
      bodyString << "),\n";  // end ClosureHint
  ...
  virtual bool TraverseForStmt(ForStmt* forStmt) {
    assert(forStmt);
    bool traverse =
        RecursiveASTVisitor<IrGLOperatorVisitor>::TraverseForStmt(forStmt);
    if (traverse) {
      bodyString << "]),\n"; // end ForAll
      bodyString << "),\n";  // end ClosureHint
      // ASSUMPTION: no code after edge-loop
  ...
  virtual bool TraverseIfStmt(IfStmt* ifStmt) {
    ...
    bool traverse =
        RecursiveASTVisitor<IrGLOperatorVisitor>::TraverseIfStmt(ifStmt);
    if (elseStmt) {
      assert(elseStmts.find(elseStmt) == elseStmts.end());
    }
    if (traverse) {
  ...
  virtual bool VisitCXXMethodDecl(CXXMethodDecl* func) {
    ...
    if (isTopological) {
      ...
      for (auto& it : accumulatorToTypeMap) {
        ...
        auto type = it.second;
        auto br   = "cub::BlockReduce<" + type + ", TB_SIZE>";
        bodyString << "CDecl([(\"__shared__ " << br << "::TempStorage\", \""
                   << name << "_ts\", \"\")]),\n";
      ...
      bodyString << "ForAll(\"" << vertexName
                 << "\", G.nodes(\"__begin\", \"__end\"),\n[\n";
      bodyString << "CDecl([(\"bool\", \"pop\", \" = " << vertexName
                 << " < __end\")]),\n"; // hack for IrGL compiler
      bodyString
          << "If(\"pop\", [\n"; // hack to handle nested parallelism (should be
                                // handled by IrGL compiler in the future)
      ...
      bodyString << "WL.pop(\"pop\", \"wlvertex\", \"" << vertexName
                 << "\"),\n";
      bodyString
          << "If(\"pop\", [\n"; // hack to handle unsuccessful pop (should be
                                // handled by IrGL compiler in the future)
      conditional_pop = true;
    }
  ...
  virtual bool VisitCXXForRangeStmt(CXXForRangeStmt* forStmt) {
    ...
    if (end == std::string::npos)
      end = vertexName.find(")", begin);
    vertexName = vertexName.substr(begin + 1, end - begin - 1);
    while (conditional > 0) {
      // ASSUMPTION: for loop is in If block, which does not have a
    ...
    bodyString
        << "UniformConditional(If(\"!pop\", [CBlock(\"continue\")]), "
           "uniform_only = False, _only_if_np = True),\n"; // hack to handle
                                                           // non-nested
                                                           // parallelism
  ...
  virtual bool VisitCallExpr(CallExpr* callExpr) {
    ...
    if (skipStmts.find(callExpr) == skipStmts.end()) {
      ...
      if (record &&
        ...
        std::string type =
            argument->getType().getUnqualifiedType().getAsString();
        parameterToTypeMap[reduceVar] = FormatType(type);
      } else {
        if (record &&
            (record->getNameAsString().compare("DynamicBitSet") == 0)) {
          auto varName = rewriter.getRewrittenText(
              cxxCallExpr->getImplicitObjectArgument()->getSourceRange());
          if (bitsetVariableToTypeMap.find(varName) ==
              bitsetVariableToTypeMap.end()) {
  ...
  virtual bool VisitIfStmt(IfStmt* ifStmt) {
    ...
    std::string text =
        FormatCBlock(rewriter.getRewrittenText(expr->getSourceRange()));
    bodyString << "If(\"" << text << "\",\n[\n";
    ++conditional;
    skipStmts.insert(expr);
    return true;
  ...
  virtual bool
  VisitMaterializeTemporaryExpr(MaterializeTemporaryExpr* tempExpr) {
  ...
  virtual bool
  VisitConditionalOperator(ConditionalOperator* conditionalOperator) {
  ...
  virtual bool VisitMemberExpr(MemberExpr* memberExpr) {
    std::string objectName =
        rewriter.getRewrittenText(memberExpr->getBase()->getSourceRange());
    const std::string& varName = memberExpr->getMemberNameInfo().getAsString();
    std::string type           = memberExpr->getMemberDecl()
                           ->getType()
                           .getUnqualifiedType()
                           .getAsString();
    if (nodeMap.find(objectName) != nodeMap.end()) {
      parameterToTypeMap[varName] = FormatType(type);
               ...
               (accumulatorToTypeMap.find(varName) ==
                accumulatorToTypeMap.end()) &&
               (reducemaxToTypeMap.find(varName) == reducemaxToTypeMap.end()) &&
               (reduceminToTypeMap.find(varName) == reduceminToTypeMap.end()) &&
               (type.find("Graph") != 0)) {
      if (globalVariableToTypeMap.find(varName) ==
          globalVariableToTypeMap.end()) {
...
class IrGLOrchestratorVisitor
    : public RecursiveASTVisitor<IrGLOrchestratorVisitor> {
private:
  ASTContext* astContext;
  Rewriter&
      rewriter; // FIXME: is this necessary? can ASTContext give source code?
...
public:
  explicit IrGLOrchestratorVisitor(ASTContext* context, Rewriter& R)
      : astContext(context), rewriter(R), requiresWorklist(false) {}
  ...
  void GenerateIrGLASTToFile() const {
    ...
    if (found != std::string::npos)
      filename = FileNamePath.substr(found + 1, FileNamePath.length() - 1);
    else
    ...
    for (auto& var : SharedVariablesToTypeMap) {
      header << "void get_bitset_" << var.first
             << "_cuda(struct CUDA_Context* ctx, uint64_t* bitset_compute);\n";
      header << "void bitset_" << var.first
             << "_reset_cuda(struct CUDA_Context* ctx);\n";
      header << "void bitset_" << var.first
             << "_reset_cuda(struct CUDA_Context* ctx, size_t begin, size_t "
                "end);\n";
      header << var.second << " get_node_" << var.first
             << "_cuda(struct CUDA_Context* ctx, unsigned LID);\n";
      header << "void set_node_" << var.first
             << "_cuda(struct CUDA_Context* ctx, unsigned LID, " << var.second
             << " v);\n";
      header << "void add_node_" << var.first
             << "_cuda(struct CUDA_Context* ctx, unsigned LID, " << var.second
             << " v);\n";
      header << "bool min_node_" << var.first
             << "_cuda(struct CUDA_Context* ctx, unsigned LID, " << var.second
             << " v);\n";
      header << "void batch_get_node_" << var.first
             << "_cuda(struct CUDA_Context* ctx, unsigned from_id, "
             << var.second << " *v);\n";
      header << "void batch_get_node_" << var.first
             << "_cuda(struct CUDA_Context* ctx, unsigned from_id, uint64_t "
                "*bitset_comm, unsigned int *offsets, "
             << var.second
             << " *v, size_t *v_size, DataCommMode *data_mode);\n";
      header << "void batch_get_mirror_node_" << var.first
             << "_cuda(struct CUDA_Context* ctx, unsigned from_id, "
             << var.second << " *v);\n";
      header << "void batch_get_mirror_node_" << var.first
             << "_cuda(struct CUDA_Context* ctx, unsigned from_id, uint64_t "
             ...
             << var.second
             << " *v, size_t *v_size, DataCommMode *data_mode);\n";
      header << "void batch_get_reset_node_" << var.first
             << "_cuda(struct CUDA_Context* ctx, unsigned from_id, "
             << var.second << " *v, " << var.second << " i);\n";
      header << "void batch_get_reset_node_" << var.first
             << "_cuda(struct CUDA_Context* ctx, unsigned from_id, uint64_t "
                "*bitset_comm, unsigned int *offsets, "
             << var.second << " *v, size_t *v_size, DataCommMode *data_mode, "
             << var.second << " i);\n";
      header << "void batch_set_mirror_node_" << var.first
             << "_cuda(struct CUDA_Context* ctx, unsigned from_id, uint64_t "
                "*bitset_comm, unsigned int *offsets, "
             << var.second << " *v, size_t v_size, DataCommMode data_mode);\n";
      header << "void batch_set_node_" << var.first
             << "_cuda(struct CUDA_Context* ctx, unsigned from_id, uint64_t "
                "*bitset_comm, unsigned int *offsets, "
             << var.second << " *v, size_t v_size, DataCommMode data_mode);\n";
      header << "void batch_add_node_" << var.first
             << "_cuda(struct CUDA_Context* ctx, unsigned from_id, uint64_t "
                "*bitset_comm, unsigned int *offsets, "
             << var.second << " *v, size_t v_size, DataCommMode data_mode);\n";
      header << "void batch_min_node_" << var.first
             << "_cuda(struct CUDA_Context* ctx, unsigned from_id, uint64_t "
                "*bitset_comm, unsigned int *offsets, "
             << var.second << " *v, size_t v_size, DataCommMode data_mode);\n";
      header << "void batch_reset_node_" << var.first
    ...
    for (auto& var : SharedVariablesToTypeMap) {
      cuheader << "\tstruct CUDA_Context_Field<" << var.second << "> "
               << var.first << ";\n";
    }
    if (requiresWorklist) {
    ...
    cuheader << "\tctx = (struct CUDA_Context* ) calloc(1, sizeof(struct "
                "CUDA_Context));\n";
    cuheader << "\tctx->id = id;\n";
    cuheader << "\treturn ctx;\n";
    cuheader << "}\n\n";
    ...
    for (auto& var : SharedVariablesToTypeMap) {
      cuheader << "\tmem_usage += mem_usage_CUDA_field(&ctx->" << var.first
               << ", g, num_hosts);\n";
    }
    cuheader << "\tprintf(\"[%d] Host memory for communication context: %3u "
    ...
    for (auto& var : SharedVariablesToTypeMap) {
      ...
      cuheader << "\tbatch_get_shared_field<" << var.second
               << ", sharedMaster, false>(ctx, &ctx->" << var.first
               << ", from_id, bitset_comm, offsets, v, v_size, data_mode);\n";
      cuheader << "}\n\n";
      ...
      cuheader
          ...
          << ", sharedMirror, true>(ctx, &ctx->" << var.first
          << ", from_id, bitset_comm, offsets, v, v_size, data_mode, i);\n";
      cuheader << "}\n\n";
  ...
  virtual bool TraverseDecl(Decl* D) {
    bool traverse =
        RecursiveASTVisitor<IrGLOrchestratorVisitor>::TraverseDecl(D);
    return traverse;
  }
  ...
  virtual bool VisitCallExpr(CallExpr* callExpr) {
    ...
    if (begin == 0) {
      ...
      for (auto decl : cxxrecord->decls()) {
        ...
        if (field) {
          auto templateType = field->getType()
                                  .getNonReferenceType()
                                  .getSingleStepDesugaredType(*astContext)
                                  .getTypePtr()
                                  ->getAs<TemplateSpecializationType>();
          if (templateType) {
            std::string type =
                templateType->getCanonicalTypeInternal().getAsString();
            ...
            } else if (type.find(__GALOIS_REDUCEMAX_TYPE__) !=
                       std::string::npos) {
              auto reducemaxName = field->getNameAsString();
              auto argType = templateType->getArg(0).getAsType().getAsString();
              reducemaxToTypeMap[reducemaxName] = argType;
            } else if (type.find(__GALOIS_REDUCEMIN_TYPE__) !=
                       std::string::npos) {
      ...
      for (auto method : cxxrecord->methods()) {
        ...
        if (method->getNameAsString().compare("operator()") == 0) {
          ...
          IrGLOperatorVisitor operatorVisitor(
              astContext, rewriter, isTopological, accumulatorToTypeMap,
              reducemaxToTypeMap, reduceminToTypeMap, SharedVariablesToTypeMap,
      ...
      std::stringstream Output_topo_nodeswithedges,
          Output_topo_nodeswithedges_body;
      ...
      if (isTopological) {
        ...
        Output_topo_masters << "Kernel(\"" << kernelName
                            << "_masterNodes_cuda\", [";
        Output_topo_masters_body << "CBlock([\"" << kernelName << "_cuda(";
        Output_topo_masters_body
            << "ctx->beginMaster, ctx->beginMaster + ctx->numOwned, ";
        Output_topo_nodeswithedges << "Kernel(\"" << kernelName
                                   << "_nodesWithEdges_cuda\", [";
        Output_topo_nodeswithedges_body << "CBlock([\"" << kernelName
                                        << "_cuda(";
      ...
      if (isTopological) {
        HostKernelsToArgumentsMap[kernelName + "_allNodes"] =
            arguments_topo_common;
        HostKernelsToArgumentsMap[kernelName + "_masterNodes"] =
            arguments_topo_common;
        HostKernelsToArgumentsMap[kernelName + "_nodesWithEdges"] =
            arguments_topo_common;
      ...
      if (!isTopological) {
        ...
        Output << "CBlock([\"ctx->in_wl.update_gpu(ctx->shared_wl->num_in_"
                  "items)\"]),\n";
      ...
      if (!isTopological) {
        ...
        Output << "CBlock([\"ctx->shared_wl->num_out_items = "
                  "ctx->out_wl.nitems()\"]),\n";
      }
      ...
      std::string fileName =
          rewriter.getSourceMgr().getFilename(cxxrecord->getLocStart()).str();
...
public:
  ...
  virtual void run(const MatchFinder::MatchResult& Results) {
    const CXXMethodDecl* decl =
        Results.Nodes.getNodeAs<clang::CXXMethodDecl>("graphAlgorithm");
    if (decl) {
      // llvm::errs() << "\nGraph Operator:\n" <<
...
public:
  IrGLFunctionsConsumer(CompilerInstance& CI, Rewriter& R)
    ...
    Matchers.addMatcher(functionDecl(hasName("go"),
