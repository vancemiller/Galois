...
__global__ void FirstItr_SSSP(CSRGraph graph, unsigned int __begin, unsigned int __end, uint32_t * p_dist_current, uint32_t * p_dist_old)
{
  unsigned tid = TID_1D;
  unsigned nthreads = TOTAL_THREADS_1D;
...
__global__ void SSSP(CSRGraph graph, unsigned int __begin, unsigned int __end, uint32_t * p_dist_current, uint32_t * p_dist_old, HGAccumulator<unsigned int> DGAccumulator_accum)
{
  unsigned tid = TID_1D;
  unsigned nthreads = TOTAL_THREADS_1D;
...
void FirstItr_SSSP_cuda(unsigned int  __begin, unsigned int  __end, struct CUDA_Context*  ctx)
{
  ...
  kernel_sizing(blocks, threads);
  // FP: "4 -> 5;
  hipLaunchKernelGGL((FirstItr_SSSP), dim3(blocks), dim3(__tb_FirstItr_SSSP), 0, 0, ctx->gg, __begin, __end, ctx->dist_current.data.gpu_wr_ptr(), ctx->dist_old.data.gpu_wr_ptr());
  // FP: "5 -> 6;
  check_cuda_kernel;
  // FP: "6 -> 7;
}
...
void SSSP_cuda(unsigned int  __begin, unsigned int  __end, unsigned int & DGAccumulator_accum, struct CUDA_Context*  ctx)
{
  ...
  *(DGAccumulator_accumval.cpu_wr_ptr()) = 0;
  ...
  // FP: "8 -> 9;
  hipLaunchKernelGGL((SSSP), dim3(blocks), dim3(__tb_SSSP), 0, 0, ctx->gg, __begin, __end, ctx->dist_current.data.gpu_wr_ptr(), ctx->dist_old.data.gpu_wr_ptr(), _DGAccumulator_accum);
  // FP: "9 -> 10;
  check_cuda_kernel;
  // FP: "10 -> 11;
...
void SSSPSanityCheck_cuda(unsigned int  __begin, unsigned int  __end, uint64_t & DGAccumulator_sum, uint32_t & DGMax, const uint32_t & local_infinity, struct CUDA_Context*  ctx)
{
  ...
  *(DGMaxval.cpu_wr_ptr()) = 0;
  ...
  // FP: "12 -> 13;
  hipLaunchKernelGGL((SSSPSanityCheck), dim3(blocks), dim3(threads), 0, 0, ctx->gg, __begin, __end, local_infinity, ctx->dist_current.data.gpu_wr_ptr(), _DGAccumulator_sum, _DGMax);
  // FP: "13 -> 14;
  check_cuda_kernel;
  // FP: "14 -> 15;
