...
__global__ void SSSP(CSRGraph graph, DynamicBitset *is_updated, unsigned int __nowned, unsigned int __begin, unsigned int __end, uint32_t * p_dist_current, uint32_t * p_dist_old, HGAccumulator<int> ret_val)
{
  unsigned tid = TID_1D;
  unsigned nthreads = TOTAL_THREADS_1D;
...
void InitializeGraph_cuda(unsigned int  __begin, unsigned int  __end, const uint32_t & local_infinity, uint64_t local_src_node, struct CUDA_Context * ctx)
{
  ...
  // FP: "4 -> 5;
  hipLaunchKernelGGL((InitializeGraph), dim3(blocks), dim3(threads), 0, 0, ctx->gg, ctx->numNodesWithEdges, __begin, __end, local_infinity, local_src_node, ctx->dist_current.data.gpu_wr_ptr(), ctx->dist_old.data.gpu_wr_ptr());
  // FP: "5 -> 6;
  check_cuda_kernel;
  // FP: "6 -> 7;
}
...
void FirstItr_SSSP_cuda(unsigned int  __begin, unsigned int  __end, struct CUDA_Context * ctx)
{
  ...
  kernel_sizing(blocks, threads);
  // FP: "4 -> 5;
  hipLaunchKernelGGL((FirstItr_SSSP), dim3(blocks), dim3(__tb_FirstItr_SSSP), 0, 0, ctx->gg, ctx->dist_current.is_updated.gpu_rd_ptr(), ctx->numNodesWithEdges, __begin, __end, ctx->dist_current.data.gpu_wr_ptr(), ctx->dist_old.data.gpu_wr_ptr());
  // FP: "5 -> 6;
  check_cuda_kernel;
  // FP: "6 -> 7;
}
...
void SSSP_cuda(unsigned int  __begin, unsigned int  __end, int & __retval, struct CUDA_Context * ctx)
{
  ...
  *(retval.cpu_wr_ptr()) = 0;
  ...
  hipLaunchKernelGGL((SSSP), dim3(blocks), dim3(__tb_SSSP), 0, 0, ctx->gg, ctx->dist_current.is_updated.gpu_rd_ptr(), ctx->numNodesWithEdges, __begin, __end, ctx->dist_current.data.gpu_wr_ptr(), ctx->dist_old.data.gpu_wr_ptr(), _rv);
  // FP: "5 -> 6;
  check_cuda_kernel;
  // FP: "6 -> 7;
...
void SSSPSanityCheck_cuda(unsigned int & sum, unsigned int & max, const uint32_t & local_infinity, struct CUDA_Context * ctx)
{
  ...
  *(maxval.cpu_wr_ptr()) = 0;
  ...
  hipLaunchKernelGGL((SSSPSanityCheck), dim3(blocks), dim3(__tb_SSSP), 0, 0, ctx->gg, ctx->beginMaster, ctx->beginMaster+ctx->numOwned, local_infinity, ctx->dist_current.data.gpu_rd_ptr(), _sum, _max);
  check_cuda_kernel;
