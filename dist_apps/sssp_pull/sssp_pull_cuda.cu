...
void SSSP_cuda(unsigned int  __begin, unsigned int  __end, unsigned int & DGAccumulator_accum, struct CUDA_Context*  ctx)
{
  ...
  *(DGAccumulator_accumval.cpu_wr_ptr()) = 0;
  ...
  // FP: "8 -> 9;
  hipLaunchKernelGGL((SSSP), dim3(blocks), dim3(__tb_SSSP), 0, 0, ctx->gg, __begin, __end, ctx->dist_current.data.gpu_wr_ptr(), (ctx->dist_current.is_updated.gpu_rd_ptr()), _DGAccumulator_accum);
  // FP: "9 -> 10;
  check_cuda_kernel;
  // FP: "10 -> 11;
